namespace _ {
  ...
  export class LiveLoopingController {
    ...
    constructor(
      ...
      this.#noteInput = this.bitwig.host
        .getMidiInPort(0)
        .createNoteInput(
          "LinnStrument",
          "?1????",
      ...
      const cursorTrack = host.createCursorTrack(
        "LINNSTRUMENT_CURSOR_TRACK",
        "Cursor Track",
        0,
        0,
        true,
      ...
      for (const noteName in noteIndexes) {
        preferences
          .getEnumSetting(
            noteName,
            "Note Colors",
            Object.keys(lightColorValues),
            noteIndexes[noteName] == 0 ? "orange" : "off",
          )
          .addValueObserver((chosenColor) => {
            if (
              this.#noteColors[noteIndexes[noteName]] !==
              (chosenColor as lightColor)
            ) {
              this.#noteColors[noteIndexes[noteName]] =
                chosenColor as lightColor;
              this.#update();
    ...
    coordinateToControlSplitButton({
      row,
      column,
    }: {
      row: number;
      ...
      return (
        this.#firstControlAreaButton +
        (7 - row) * this.#getPlayAreaWidth() +
        column
      );
    }

    controlSplitButtonToCoordinate(button: number): {
      row: number;
      column: number;
    } {
      const row =
        7 -
        Math.floor(
          (button - this.#firstControlAreaButton) / this.#getPlayAreaWidth(),
        );
    ...
    #buttonToNote(buttonIndex: number): number {
      ...
      const note =
        buttonIndex - rowAdjustment + this.#noteOffset + interfaceOffset;
    ...
    handleMidi(status: number, data1: number, data2: number) {
      ...
      this.#modules[this.#currentMode].some((module) =>
        module.handleMidi({ type, channel, data1, data2 }),
      );
    }
    ...
    setLight(options: {
      ...
      const linnOptions = {
        row: options.row,
        column:
          options.column + (options.absolute ? 0 : this.#getLeftSplitWidth()),
        color: options.color,
      };
